...
const (
	...
	firstFrameTimeout     = 8 * time.Second
)

var errFirstFrameTimeout = errors.New("camera did not deliver a frame")
...
func Discover() Inventory {
	...
	if result.CaptureCount > 0 {
		result.Message = "Camera capture node detected. Open live view " +
			"to verify that the sensor delivers frames."
...
func readFirstChunk(
	...
	go func() {
		...
		if count > 0 {
			result <- readResult{data: buffer[:count]}
			return
		}
		if err == nil {
