...
const guides = [
  ...
  guide(
    ...
    [
      ...
      step(
        "Prove frame delivery",
        "Inspect the output too. Some drivers return zero" +
          " even after reporting a select timeout.",
        "timeout 10 v4l2-ctl -d /dev/video<N> --stream-mmap " +
          "--stream-poll --stream-count=1 --stream-to=/dev/null",
      ),
      step(
        "Run a GStreamer smoke test",
        "Require 30 frames to cross the complete capture p" + "ath.",
        "timeout 10 gst-launch-1.0 -q v4l2src " +
          "device=/dev/video<N> num-buffers=30 ! fakesink",
      ),
      step(
    ...
    "Both frame tests finish before their deadlines," +
      " a stable image appears, and no driver error is reported.",
  ),
  guide(
    "audio",
    "AUD",
  ...
  guide(
    ...
    [
      ...
      step(
        ...
        "Run the BSP model repeatedly through the Vivante" + " delegate.",
        "cd /usr/bin/tensorflow-lite-*/examples && " +
          "./label_image --external_delegate_path=" +
          "/usr/lib/libvx_delegate.so " +
          "-m mobilenet_v1_1.0_224_quant.tflite " +
          "-i grace_hopper.bmp -l labels.txt -c 100 -w 1 -r 5",
      ),
    ],
